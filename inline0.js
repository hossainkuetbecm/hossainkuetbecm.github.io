    ...
    function applyUploadedUrl(target, url) {
      ...
      if (target === 'projectImage') { $('#image').value = url; updatePreview(); return 'Selected project image updated.'; }
    ...
    function newProject() {
      selectedId = '';
      fillForm({ id:'', title:'', tag:'Project', category:(categories[0] && categories[0].id) || '', year:new Date().getFullYear(), scheduleDate:today(), image:'', description:'', responsibilities:[], deliverables:[], tools:[], isPublished:true });
      renderProjectList();
    }
